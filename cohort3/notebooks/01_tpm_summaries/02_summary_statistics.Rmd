...
Summary statistics for S13 expression from @monaco_rna-seq_2019.
...
There are two cell type label schemes (a.k.a. total K, or total cell types), 
one, the original, which contains 17 cell types. The second, the reduced mapping, 
contains 8 cell types.

## Table of mappings

The comparison of mappings looks as follows:
...
```{r}
hmRefNew <- Heatmap(
  scale(zrefMapped), show_row_names = FALSE, name = heatmap.label)
hmRefNew
```
...
Barplots, original cell sizes

```{r}
bpSizeOriginal <- ggplot() + geom_bar()
```

Barplots, new cell sizes

```{r}
bpSizeNew <- ggplot() + geom_bar()
```

Barplots, original cell sizes and new cell sizes

```{r}
grid.arrange(bpSizeOriginal,bpSizeNew,nrow=2)
```
