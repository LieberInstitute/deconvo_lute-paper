...
for(type in colnames(dfProportions)){
  for(sample.id in unique(dfPlotTallTall$sample.id)){
    filterTall <- 
      dfPlotTall[
        dfPlotTall[,"sample.id.format"]==sample.id, type]$true.proportions <- 
      dfProportions[sample.id,type]
  }
}


# append to df.plot.tall.s13
dfPlotTall$true.proportions <- "NA"
dfPlotTall$sample.id.format <- gsub("_.*", "", dfPlotTall$sample.id)
for(type in colnames(df.proportions)){
  for(sample.id in unique(dfPlotTall$sample.id.format)){
    filterTall <- 
      dfPlotTall[
        dfPlotTall[,"sample.id.format"]==sample.id, type]$true.proportions <- 
      dfProportions[sample.id,type]
