...
order_svector <- function(s.vector = c("neuron" = 10, "glial" = 3)){s.vector[names(s.vector)[order(names(s.vector))]]}
...
get_ypb_experiment_results <- function(sce, sample.id.variable = "Sample", 
  ...
  dfp <- do.call(rbind, lapply(unique.sample.id.vector, function(sample.id){
    ...
    ypb.iter <- ypb_from_sce(singleCellExperiment = sce.iter, assayName = assay.name, 
