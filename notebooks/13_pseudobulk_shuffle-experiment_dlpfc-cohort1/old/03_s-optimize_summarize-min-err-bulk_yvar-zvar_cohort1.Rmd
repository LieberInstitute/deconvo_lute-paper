...
* Z is varied with sample ID (from a single snRNAseq sample).

* Y is varied with sample ID(from real RNAseq data)
...
Gets one unique minimum neuron error per data label.

```{r}
length(unique(df.res$sample.label))
...
```{r}
# filter outlying high error
dim(df.min)
df.min <- df.min[df.min$min.error.neuron<0.1,]
dim(df.min)
```

## Example field gradient heatmaps and highlight plots

Shows example heatmaps and highlight plots for label IDs having the lowest and highest observed minmimum neuron errors.

```{r}
# subset/filter
df.min.filter <- df.min$min.error.neuron==max(df.min$min.error.neuron)
sample.label.highest <- df.min[df.min.filter,]$sample.label[1]
df.res.filter <- df.res$sample.label==sample.label.highest
df.res.highest <- df.res[df.res.filter,]
# plot
list.plots.dfp1 <- deconvo_plots_list(df.res.highest, "sample.label")
...
# Plots across bulk condition groups
...
## Plots by `library.type`

```{r}
df.min$library.prep <- gsub(".*_", "", df.min$cell.compartment)
ggplot(df.min, aes(x = library.type, y = min.error.neuron)) + 
  geom_jitter(alpha = 0.5) + geom_boxplot(alpha = 0, color = "cyan")
```
...
```{r}
df.min$library.prep.x.cell.compartment <- paste0(df.min$library.type, ";",df.min$cell.compartment)
