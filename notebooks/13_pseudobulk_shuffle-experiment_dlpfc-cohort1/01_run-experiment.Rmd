...
df.res.samples.pt2 <- do.call(rbind, lapply(sample.id.vector, function(sample.id){
  # remove matched sample
  filter.sce.iter <- sce[["sample.id"]]==sample.id
  sce.iter <- sce[,!filter.sce.iter]
  
  # assign true df list
  colnames(y.unadj) <- sample.id
  
  # 
  names(list.df.true) <- sample.id
  
  df.res.samples.pt2 <- multigroup_bias_matched(
    sample.id.vector, list.df.true, y.unadj, 
