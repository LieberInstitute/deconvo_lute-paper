...
df.res.samples.pt2 <- do.call(rbind, lapply(sample.id.vector, function(sample.id.iter){
  ...
  df.res <- multigroup_bias_matched(
...
df.res.samples.pt2 <- multigroup_bias_matched(
  sample.id.vector, list.df.true, y.unadj, dfs.rn, sce, assay.name = assay.name)
df.res.samples.pt2$sn.rn.sample.id <- rep(dfs.rn$sample.id, length(sample.id.vector))
