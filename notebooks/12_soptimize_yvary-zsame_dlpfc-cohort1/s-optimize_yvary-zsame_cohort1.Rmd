...
# Plot results, first Y of type "block_position_type"
...
# Plots across Y blocks -- all Bulk_polyA samples

This is the filter step

```{r}
df.res.filter <- df.res$cell.compartment=="Bulk_polyA"
df.plot <- df.res[df.res.filter,]
```
...
```{r}
ggplot(df.res, aes(x = glial, y = neuron, color = minimum.error)) + 
  geom_point() + scale_color_manual(values = c("TRUE" = highlight.color.low, 
                                               "FALSE" = background.color.highlights)) +
  facet_wrap(~sample.id)
```

Plot heatmap -- error.neuron, lowest decile

```{r}
```

Plot heatmap -- error.neuron, highest value

```{r}
```

Plot heatmap -- error.neuron, highest decile
