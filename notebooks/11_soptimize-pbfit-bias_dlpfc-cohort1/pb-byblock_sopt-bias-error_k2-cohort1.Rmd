...
Line plots of K2_1 by K2_1 bias, color by block ID.

```{r}
# neuron
ggplot(df.res, aes(x = neuron, y = bias.neuron.true.pred, group = glial.group.label)) + 
  geom_point(alpha = 0.2) + geom_line() + theme_bw() + facet_wrap(~sample.id) + geom_hline(yintercept = 0) +
  ggtitle("Neuron")
