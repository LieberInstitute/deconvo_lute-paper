...
k2.1a.result.path <- "k2_1a_result_list.rda"
...
```{r}
list.df.true.1b <- lapply(list.df.true, function(item){return(NULL)})
names(list.df.true.1b) <- names(list.df.true.1b)
k2.1b.result.list <- kmatch_experiment(k.variable.name = k.variable.name,
                              sce = sce, 
                              sample.id.vector = sample.id.vector, 
                              list.df.true = list.df.true.1b,
                              y.eset = y.unadj, y.train = y.train, 
                              y.validate = y.validate, 
                              assay.name = assay.name, 
                              group.name = group.name)

k2.1b.result.path <- "k2_1b_result_list.rda"
save(k2.1b.result.list, file = k2.1b.result.path)
```
