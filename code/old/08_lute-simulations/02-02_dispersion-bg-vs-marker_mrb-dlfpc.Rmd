...
Compare dispersions by donor-type.

```{r}
# aggregate data into sce object
sce[["donor-type"]] <- paste0(sce[["donor"]], "-", sce[["k2"]])
sce.agg <- aggregateAcrossCells(sce, ids = sce[["donor-type"]], 
                                statistics = c("mean", "median"), 
                                use.assay.type = "logcounts_ds")

# get dispersion results
ld.agg.mean <- sce_dispersion(sce.agg, group.data = "k2", 
                              assayname = "logcounts_ds.mean",
                              highlight.markers = dfmt$gene, downsample = FALSE,
                              hl.color = "red", point.alpha = 0.01,
                              hl.alpha = 1, ref.linecol = "black")
ld.agg.median <- sce_dispersion(sce.agg, group.data = "k2",
                                assayname = "logcounts_ds.median",
                                highlight.markers = dfmt$gene, 
                                downsample = FALSE,
                                hl.color = "red", point.alpha = 0.01,
                                hl.alpha = 1, ref.linecol = "black")

# show dispersion plots
ld.agg.mean$ggplot.dispersion
ld.agg.median$ggplot.dispersion
```
