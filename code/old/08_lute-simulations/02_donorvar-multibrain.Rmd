...
```{r}
typevar <- "is.excit"
scef[[typevar]] <- ifelse(grepl("^Excit.*", scef[["cellType"]]), TRUE, FALSE)
groupstatv <- c("count", "zerocount", "var")
dfs.short <- sce_groupstat(scef, group.variable = "donor", type.variable = typevar,
                     groupstat = groupstatv, summarytype = "colData",
                     return.tall = FALSE, assayname = "logcounts")
dfs.tall <- sce_groupstat(scef, group.variable = "donor", type.variable = typevar,
                     groupstat = groupstatv, summarytype = "colData",
