...
halo.cellsize <- aggregate(halo.k2[,cell.area.variable], 
...
halo.cellsize[halo.cellsize$k2=="glial",]$norm.size <- 
  halo.cellsize[halo.cellsize$k2=="glial",]$median.cellsize/
  halo.cellsize[halo.cellsize$k2=="neuron",]$median.cellsize
...
result.table.name <- "deconvo-results-table_bulk-matched-sn-bydonor_s-halo-cellsize-normbyneuron_scale-yz-logcounts_train.rda"
...
result.table.list <- get(load("./deconvo_method-paper/outputs/20_snrnaseq-bulk-matched_training/deconvo-results-table_bulk-matched-sn-bydonor_s-halo-cellsize-normbyneuron_scale-yz-logcounts_train.rda"))
# image reference
halo.path <- "Human_DLPFC_Deconvolution/processed-data/03_HALO/halo_all.Rdata"
halo.all <- get(load(halo.path))
# filter halo.all
...
# error summaries
median(result.filter[result.filter$scale==T,]$abs.error.neuron) # 0.1595581
median(result.filter[result.filter$scale==F,]$abs.error.neuron) # 0.1722251
median(result.filter[result.filter$scale==T,]$abs.error.glial) # 0.3983474
...
plot.data <- do.call(rbind, lapply(sample.id.vector, function(sample.id){
  ...
  value.neuron <- dff[dff$scale==FALSE,"abs.error.neuron"]-dff[dff$scale==TRUE,"abs.error.neuron"]
  value.glial <- dff[dff$scale==FALSE,"abs.error.glial"]-dff[dff$scale==TRUE,"abs.error.glial"]
  c(sample.id, value.neuron, value.glial, dff$halo.neuron.cells[1])
})) %>% as.data.frame()
colnames(plot.data) <- c("sample.id", "scale.error.diff.neuron", 
                         "scale.error.diff.glial", "halo.cells.neuron")
for(c in c(2,3,4)){plot.data[,c] <- as.numeric(plot.data[,c])}
plot.data[,1] <- factor(plot.data[,1], levels = plot.data[,1][order(plot.data[,2])])
# barplot error diff
ggplot(plot.data, aes(x = sample.id, y = scale.error.diff.neuron)) + 
  geom_bar(stat = "identity") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggplot(plot.data, aes(x = sample.id, y = scale.error.diff.glial)) + 
  geom_bar(stat = "identity") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
