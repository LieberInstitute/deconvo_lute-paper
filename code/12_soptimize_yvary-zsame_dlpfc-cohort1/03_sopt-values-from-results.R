...
df.min <- do.call(rbind, lapply(labels.vector, function(label.iter){
  ...
  df.iter[df.iter$error.neuron == min(df.iter$error.neuron)[1],,drop=F]
}))
...
list.sopt <- lapply(labels.vector, function(label.iter){
  df.iter <- df.min[df.min$sample.labels==label.iter,]
...
dfp <- rbind(data.frame(s.train = df.min$glial), data.frame(s.train = df.min$neuron))
...
# scatter
ggplot(df.min, aes(x = glial, y = neuron)) + geom_point(alpha = 0.5) +
