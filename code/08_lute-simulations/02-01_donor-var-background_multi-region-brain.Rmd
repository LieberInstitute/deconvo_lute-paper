...
df <- do.call(rbind, lapply(ctv, function(ci){
  do.call(rbind, lapply(dv, function(di){
    filt <- sce[["ctbroad"]]==ci & sce[["donor"]]==di
    me <- logcounts(sce[,filt])
    df <- data.frame(var = rowVars(me, na.rm = T), 
                     mean = rowMeans(me, na.rm = T))
    df$ct <- ci; df$donor <- di
    df
  }))
}))
```
