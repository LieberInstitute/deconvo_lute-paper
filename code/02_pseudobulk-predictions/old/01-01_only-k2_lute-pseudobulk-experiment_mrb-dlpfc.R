...
# get pseudobulks
ypb.k2 <- ypb_from_sce(sce = sce.k2, assay.name = "logcounts", celltype.variable = "k2", 
                       sample.id.variable = "Sample", S = c("glial" = 3, "neuron" = 10)) %>% as.matrix()
...
ggplot(dfp.k2, aes(x = neuron.true, y = neuron.pred)) + 
  geom_point() + 
  geom_abline(slope = 1, intercept = 0) + geom_hline(yintercept = 0.5) + geom_vline(xintercept = 0.5) + theme_bw() +
