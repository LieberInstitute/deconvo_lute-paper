...
datv <- sample(10, 4*10, replace = T) + 1
```
...
```{r}
# run experiment
pb.expt <- get_pb_experiment(lz = lexpt, scef = sef, datv = datv,
                             scale.range = 100:10000, ctvarname = "celltype.treg",
                             plot.results = TRUE, method.str = "nnls",
                             plot.save.dpath = file.path(proj.dpath, "outputs",
                                                         "03_test-stransform"),
                             save.plots = FALSE, seed.num = 2)
```
...
```{r}
ggpt.main <- ggplot(df.tall, aes_string(x = pi_true, y = pi_est),
