...
new.cell.type.variable <- "k4"
cell.type.vector <- sce[["cellType_broad_hc"]]
new.cell.type.vector <- ifelse(cell.type.vector %in% c("Excit"), "Excit",
                               ifelse(cell.type.vector %in% c("Inhib"), "Inhib",
                                      ifelse(cell.type.vector %in% c("OPC", "Astro", "Micro"), 
                                             "glial", 
                                             ifelse(cell.type.vector == "Oligo", "oligo", "other"))))
sce[[new.cell.type.variable]] <- new.cell.type.vector
# remove "other"
filter.other <- !sce[[new.cell.type.variable]]=="other"
sce <- sce[,filter.other]

markers.by.group <- markers_by_group(sce, 
                                     group.variable = "Sample", 
                                     celltype.variable = new.cell.type.variable, 
...
markers.filtered.path <- file.path(paste0(
  "deconvo_method-paper/outputs/overlap-markers-filtered-",
  new.cell.type.variable,"_train.rda"))
save(markers.filtered, file = markers.filtered.path)

# rank markers overall
markers.overall <- meanratiosParam(sce, "logcounts", new.cell.type.variable, 
...
markers.vector.overall.top.path <- file.path(paste0(
  "deconvo_method-paper/outputs/overall-markers-filtered-",
  new.cell.type.variable,"_train.rda"))
...
# save
se.markers.name <- paste0("se-",new.cell.type.variable,
                          "_markers-overlap-concordance-filter-final_train.rda")
