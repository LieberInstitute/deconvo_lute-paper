...
sce.k4 <- sce.mrb[rownames(sce.mrb) %in% rownames(list.sce.markers[["k4"]]),]
s.vector <- c("Excit" = 10, "Inhib" = 10, "non_oligo_glial" = 3, "Oligo" = 3)



# get experiment results tables
dfp.tall <- get_ypb_experiment_series(sce.k4, sample.id.variable = "donor", 
                                      celltype.variable = "k4", assay.name = "logcounts",
                                      s.vector = s.vector,
                                      algorithm.name = "nnls", return.dimensions = "tall")
dfp.wide <- get_ypb_experiment_series(sce.k4, sample.id.variable = "donor", 
                                      celltype.variable = "k4", assay.name = "logcounts",
                                      s.vector = s.vector,
...
# save env
save.image("./cohort2/env/01_pseudobulk/01_k4_mrb_script.RData")
