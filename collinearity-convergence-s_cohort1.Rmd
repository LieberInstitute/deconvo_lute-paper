...
list.results <- lapply(seq(nrow(dfs)), function(index){
  ...
  zlist.donors <- z_list_bydonor(sce, sample.id.variable = "Sample", 
                           s.vector = c("glial" = 3, "neuron" = 10), 
                           assay.name = "logcounts", celltype.variable = "k2")
  covcor.types <- covcor_from_zlist(zlist.types, type = "cor")
  covcor.donors <- covcor_from_zlist(zlist.donors, type = "cor")
...
```{r}
ggplot(df.covar, aes(x = s.neuron, y = covariance.diff.unadj.minus.adj)) + 
