...
list.results <- lapply(seq(nrow(dfs)), function(index){
  ...
  zlist.types <- z_list(sce, s.vector, assay.name = "logcounts", celltype.variable = "k2")
  zlist.donors <- z_list_bydonor(sce, sample.id.variable = "Sample", s.vector = s.vector, 
                           assay.name = "logcounts", celltype.variable = "k2")
  covcor.types <- covcor_from_zlist(zlist.types)
  covcor.donors <- covcor_from_zlist(zlist.donor)
  list(zlist.types = zlist.types, zlist.donors = zlist.donors, 
       covcor.types = covcor.types, covcor.donors = covcor.donors)
})
...
df.covar <- do.call(rbind, lapply(seq(length(list.results)), function(index){
  df.iter <- list.results[[index]][["covcor.donors"]][["lcov.diff.unadj.minus.adj"]] %>% melt()
...
```{r}
summary(df.covar[
  df.covar$donor=="Br2720_mid" & 
    df.covar$compare.label=="glial;neuron",]$covariance.diff.unadj.minus.adj)
```
