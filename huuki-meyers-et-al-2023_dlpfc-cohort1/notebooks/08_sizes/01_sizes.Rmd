...
```{r}
table.s6 <- dfp.wide
table.s6 <- table.s6[,c(6,7,1,2,10,3,4,11)]
colnames(table.s6) <- gsub("\\.", "_", colnames(table.s6))
for(c in c(3:4)){table.s6[,c] <- round(table.s6[,c], 0)}
for(c in c(5:8)){table.s6[,c] <- round(table.s6[,c], 2)}
table.s6[,1] <- ifelse(table.s6[,1]=="mid", "middle",
                       ifelse(table.s6[,1]=="ant", "anterior", "posterior"))
colnames(table.s6)[1] <- "dlpfc_region"
```

```{r}
setwd("..")
setwd("..")
write.csv(table.s6, file = "./outputs/08_sizes/TABLE_S6.csv", row.names = FALSE)
```
